# TODO: needs love!
base_places_rt = {
    "trapped_place": ["slaughterhouse", "cages", "prison", "trap", "snare", "jail", "#threatening_substance#", "time"],

    "moment_in_time": ["#day_of_the_week#", "age", "era", "aeon", "eon", "day",
                       "night", "year", "solstice", "time", "dawn", "sunrise",
                       "dusk", "twilight", "forever", "tomorrow", "future"],

    "spooky_locale": ["swamp", "island", "land", "park", "beach", "lake", "tower", "mountain", "hill", "lighthouse", "pyramid", "void", "barrens", "barrier", "caldera", "castle", "cave", "continent", "crater", "crypt", "dungeon", "field", "forest", "graveyard", "cemetery", "grotto", "hill", "hive", "island", "jungle", "keep", "kingdom", "lagoon", "lagoon", "lake", "mausoleum", "mine", "mountain", "rim", "ruin", "sands", "sepulcher", "shipwreck", "shores", "swamp", "throne", "tomb", "torture chamber", "tower", "valley", "vault", "volcano", "wilderness", "fortress", "labrynth", "pit", "earth", "year #funny_year#", "41st Millennium", "center of the earth", "below", "beyond", "above", "underground", "20,000 fathoms"],

    "building": ["cabin", "village", "hotel", "house", "mansion", "fort",
                 "stalag", "base", "barracks", "town", "colony", "cabin",
                 "city", "realm", "zone", "church", "abbey", "cathedral",
                 "pagoda", "chapel", "reliquary", "synagogue", "mosque",
                 "temple", "hostel", "bodega", "adult cinema", "suburbs",
                 "fair", "barn", "mall", "science fair", "DMV", "library"],

    "water_and_oceans": [ "sea", "deep", "depths", "reef"],

    "cosmic_place": ["asteroid belt", "moon", "black hole", "eclipse", "continuum", "#star# constellation", "planet", "galaxy", "nebula", "dimension", "world", "star", "solar system"],

    "scary_parts_of_a_house": [
        "attic", "basement", "bed", "cellar", "closet", "fireplace",
        #"floorboards",
        #"painting",
        "furnace", 
        "kitchen", 
        #"room",
        "sewer", 
        "garden", 
        #"backyard",
        "shed", 
        #"stairs", 
        "walls", 
        "well", 
        "crawlspace", 
        "panic room"
    ],

    "places_on_earth_that_use_the": [
                        "Orient", "atlantic", "far #cardinal_direction#",
                        "nile", "north pole", "outback",
                        "Himalayas", "pacific", "antarctic", "Zambezi",
                        "amazon"],

}

base_places_nrt = {

    # NOTE: proper names
	"star": [ "altair", "andromeda", "cygnus", "orion", "corona borealis", "cetus", "sol", "centauri"],
    "fancy_star": ["#star# #maybe_fun_numeral#", "#greek_letter# #star#"],
    "named_planetary_body": ["phobos", "io", "mars", "mercury", "neptune", "jupiter", "pluto", "saturn", "titan", "uranus", "venus", "planet x",],
    "mythological_places": ["styx", "kingdom come", "hell", "dune", "elm street", "greenwich village", "eden", "atlantis", "troy", "Olympus", "Armageddon", "valhalla", "underworld", "astral plane", "miskatonic university", "Innsmouth", "avalon", "Ga'Hoole", "Shangra-La", "El Dorado", "Hyboria", "carcosa", "R'lyeh", "#greek_letter# Ceti #number#", "#greek_letter# Persei #number#", "vulcan", "lemuria", "leng"],
    "places_on_earth": ["Manhattan", "Antarctica", "Babylonia", "Basque Country", "Africa", "Baghdad", "Persia", "Harlem", "Berlin", "Bavaria", "Egypt", "L.A.", "New Jersey", "Tibet", "Area 51", "Groom Lake", "Main Street USA", "Mesoptamia", "Mongolia", "Siberia", "Soviet Russia", "Tampa", "Yucca Flats", "#cardinal_direction#", "Rutgers"],
    "other": ["time", "space"],


}
